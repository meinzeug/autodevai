...
impl HighPerformanceCache {
    pub fn new(config: CacheConfig) -> Self {
        ...
        info!(
            "High-performance cache initialized with L1: {}MB, L2: {}MB, L3: {}MB",
    ...
    pub async fn get(&self, key: &str) -> Option<Vec<u8>> {
        ...
        if result.is_none() {
            ...
            if let Some(entry) = l2_cache.get_mut(key) {
                if !self.is_expired(entry) {
                    ...
                    // Promote to L1
                    let mut l1_cache = self.l1_cache.write().await;
                    self.ensure_l1_capacity(&mut l1_cache, entry.compressed_size)
                        .await;
        ...
        if result.is_none() {
            ...
            if let Some(entry) = l3_cache.get_mut(key) {
                if !self.is_expired(entry) {
                    ...
                    // Promote to L2
                    let mut l2_cache = self.l2_cache.write().await;
                    self.ensure_l2_capacity(&mut l2_cache, entry.compressed_size)
                        .await;
        ...
        self.update_access_metrics(key, cache_level, result.is_some(), access_time)
            .await;
    ...
    pub async fn set(
        ...
        debug!(
            "Stored key: {} in L1 cache with compression ratio: {:.2}",
    ...
    async fn ensure_l1_capacity(
        &self,
        l1_cache: &mut HashMap<String, CacheEntry>,
        required_size: usize,
    ) {
        ...
        if current_size + required_size > max_size {
            let to_remove = self
                .select_eviction_candidates(l1_cache, required_size)
                .await;
    ...
    async fn ensure_l2_capacity(
        &self,
        l2_cache: &mut HashMap<String, CacheEntry>,
        required_size: usize,
    ) {
        ...
        if current_size + required_size > max_size {
            let to_remove = self
                .select_eviction_candidates(l2_cache, required_size)
                .await;
            for key in to_remove {
                if let Some(entry) = l2_cache.remove(&key) {
                    ...
                    if !self.is_expired(&entry) {
                        let mut l3_cache = self.l3_cache.write().await;
                        self.ensure_l3_capacity(&mut l3_cache, entry.compressed_size)
                            .await;
    ...
    async fn ensure_l3_capacity(
        &self,
        l3_cache: &mut HashMap<String, CacheEntry>,
        required_size: usize,
    ) {
        ...
        if current_size + required_size > max_size {
            let to_remove = self
                .select_eviction_candidates(l3_cache, required_size)
                .await;
            for key in to_remove {
                l3_cache.remove(&key);
            }
    ...
    async fn select_eviction_candidates(
        &self,
        cache: &HashMap<String, CacheEntry>,
        _required_size: usize,
    ) -> Vec<String> {
    ...
    async fn update_access_metrics(
        &self,
        key: &str,
        cache_level: &str,
        hit: bool,
        ...
        if hit {
            ...
            match cache_level {
                "L1" => {
                    metrics.l1_hit_rate =
                        (metrics.l1_hit_rate * (metrics.total_requests - 1) as f64 + 1.0)
                            / metrics.total_requests as f64
                }
                "L2" => {
                    metrics.l2_hit_rate =
                        (metrics.l2_hit_rate * (metrics.total_requests - 1) as f64 + 1.0)
                            / metrics.total_requests as f64
                }
                "L3" => {
                    metrics.l3_hit_rate =
                        (metrics.l3_hit_rate * (metrics.total_requests - 1) as f64 + 1.0)
                            / metrics.total_requests as f64
                }
                _ => {}
            }
        ...
        metrics.overall_hit_rate =
            (metrics.total_hits as f64 / metrics.total_requests as f64) * 100.0;

        // Update average access time
        let access_time_ms = access_time.as_millis() as f64;
        metrics.average_access_time_ms =
            (metrics.average_access_time_ms * (metrics.total_requests - 1) as f64 + access_time_ms)
                / metrics.total_requests as f64;
        ...
        if self.config.analytics_enabled {
            ...
            entry.average_access_time_ms =
                entry.total_access_time_ms / (entry.hit_count + entry.miss_count) as f64;
        }
    ...
    pub async fn get_metrics(&self) -> CacheMetrics {
        ...
        metrics.l1_size_mb =
            l1_cache.values().map(|e| e.compressed_size).sum::<usize>() as f64 / (1024.0 * 1024.0);
        metrics.l2_size_mb =
            l2_cache.values().map(|e| e.compressed_size).sum::<usize>() as f64 / (1024.0 * 1024.0);
        metrics.l3_size_mb =
            l3_cache.values().map(|e| e.compressed_size).sum::<usize>() as f64 / (1024.0 * 1024.0);

        // Calculate overall compression ratio
        let total_original: usize = l1_cache
            .values()
            .chain(l2_cache.values())
            .chain(l3_cache.values())
            .map(|e| e.original_size)
            .sum();
        let total_compressed: usize = l1_cache
            .values()
            .chain(l2_cache.values())
            .chain(l3_cache.values())
...
mod tests {
    ...
    async fn test_cache_operations() {
        ...
        cache
            .set("test_key", data.clone(), None, vec!["test".to_string()])
            .await
            .unwrap();
    ...
    async fn test_cache_eviction() {
        ...
        for i in 0..100 {
            ...
            cache
                .set(&format!("key_{}", i), data, None, vec![])
                .await
                .unwrap();
        }
