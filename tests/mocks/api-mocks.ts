...
export class APIMocks {
  ...
  private mocks: Map<string, vi.MockedFunction<any>>;
  ...
  mockClaudeFlowInit(response?: Partial<MockResponse>): vi.MockedFunction<any> {
  ...
  mockClaudeFlowSpawnAgent(response?: Partial<MockResponse>): vi.MockedFunction<any> {
    const mockFn = vi.fn().mockResolvedValue({
      success: true,
  ...
  mockClaudeFlowOrchestrate(response?: Partial<MockResponse>): vi.MockedFunction<any> {
    const mockFn = vi.fn().mockResolvedValue({
      success: true,
  ...
  mockClaudeFlowStatus(response?: Partial<MockResponse>): vi.MockedFunction<any> {
    const mockFn = vi.fn().mockResolvedValue({
      success: true,
      status: 'active',
      agents: [
  ...
  // Codex API Mocks
  mockCodexExecute(response?: Partial<MockResponse>): vi.MockedFunction<any> {
    const mockFn = vi.fn().mockImplementation((request) => {
  ...
  mockCodexExecuteError(errorMessage: string = 'Execution failed'): vi.MockedFunction<any> {
    const mockFn = vi.fn().mockResolvedValue({
      success: false,
      error: errorMessage,
      status: 'failed'
  ...
  // Docker API Mocks
  mockDockerCreate(response?: Partial<MockResponse>): vi.MockedFunction<any> {
    const mockFn = vi.fn().mockResolvedValue({
      success: true,
  ...
  mockDockerList(response?: Partial<MockResponse>): vi.MockedFunction<any> {
    const mockFn = vi.fn().mockResolvedValue({
      success: true,
      containers: [
        {
  ...
  mockDockerStop(response?: Partial<MockResponse>): vi.MockedFunction<any> {
    const mockFn = vi.fn().mockResolvedValue({
      success: true,
      status: 'stopped',
      ...response
  ...
  mockDockerRemove(response?: Partial<MockResponse>): vi.MockedFunction<any> {
    const mockFn = vi.fn().mockResolvedValue({
      success: true,
      status: 'removed',
      ...response
  ...
  mockStateSave(response?: Partial<MockResponse>): vi.MockedFunction<any> {
    const mockFn = vi.fn().mockResolvedValue({
      success: true,
  ...
  mockStateLoad(response?: Partial<MockResponse>): vi.MockedFunction<any> {
    const mockFn = vi.fn().mockResolvedValue({
      success: true,
      data: {
  ...
  // External API Mocks
  mockOpenRouterAPI(response?: any): vi.MockedFunction<any> {
    const mockFn = vi.fn().mockResolvedValue({
      choices: [
        {
          message: {
  ...
  mockServerError(statusCode: number = 500): vi.MockedFunction<any> {
  ...
  // Utility Methods
  getMock(name: string): vi.MockedFunction<any> | undefined {
